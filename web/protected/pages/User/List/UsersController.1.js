...
PageJs.prototype = Object.extend(new BPCPageJs(), {
	...
	,_getItemRow: function(item, isTitle) {
		...
		if(tmp.isTitle !== true) {
			...
			//btnsdiv
			tmp.btnDiv
				.insert({'bottom': new Element('span', {'id': 'edit_btn_' + item.id, 'class': 'btn btn-default btn-sm edit', 'data-loading-text': 'Loading...'})
					.update(new Element('span', {'class': 'glyphicon glyphicon-pencil', 'title': 'Edit this user'}))
					.observe('click', function() {
						tmp.me._showEditPanel(item);
					})
				})
				.insert({'bottom': new Element('span', {'id': 'del_btn_' + item.id, 'class': 'btn btn-danger btn-sm delete', 'data-loading-text': 'Processing...'})
